...
class Admin(meta.Declarative):
    ...
    def get_hash(cls, password):
        salt = hashlib.sha512(str(g.settings.security.hash.salt).encode(
            'utf-8')).hexdigest()
    ...
    def get(cls, login, password):
        admin = cls.query.filter(cls.login == login).filter(
            cls.password == cls.get_hash(password)).first()
        return admin
    ...
    def valid(self):
        ...
        """
        self2 = self.__class__.query.filter(
            self.__class__.admin_id == self.id).first()
        if self2:
    ...
    def reload_permissions(self):
        permissions = AdminPermission.query.filter(
            AdminPermission.admin_id == self.id).all()
        self.global_permissions, self.board_permissions = g.permissions.process_admin_permissions(
            permissions)
    ...
    def delete_permission(self, ap_id):
        ap = AdminPermission.query.filter(
            AdminPermission.admin_id == self.id).filter(
                AdminPermission.admin_permissions_id == ap_id).first()
        if ap:
    ...
    def has_permission(self, name, scope=0):
        ...
        if name in self.global_permissions:
            return True
        elif scope and scope in self.board_permissions and name in self.board_permissions[
                scope]:
            return True
        else:
            return False
