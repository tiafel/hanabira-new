...
class MusicFile(Filetype):
    ...
    gettag = {'VCFLACDict': flac_tag,
              'ID3': id3_tag,
              'OggVCommentDict': oggv_tag}
    ...
    def process(self, file, thumb_res, fileset):
        ...
        if thumb:
            ...
            file.thumbnail = self.make_thumb(thumb, thumb_path, 'png',
                                             thumb_res)
    ...
    def process_file(self, filepath, extension):
        ...
        if 'bitrate' in meta.info.__dict__:
            ...
            metadata[
                'bitrate'] = meta.info.total_samples * meta.info.bits_per_sample / meta.info.length
                ...
                if taglist:
                    for tag2 in taglist:
                        if tag2 in meta.tags:
                            metadata[tag], encoding = fix_charset_unicode(
                                gettag(tag2, meta.tags), encoding)
                            if encoding and encoding != 'utf-8' or meta.tags[
                                    tag2].encoding != 3:
                                meta.tags[tag2].encoding = 3
                                meta.tags[tag2].text[0] = metadata[tag]
                                recoded = True
                                log.info("%s/%s => %s (%s) [%s]" %
                                         (tag, tag2, metadata[tag],
                                          type(metadata[tag]), encoding))
                            break
                else:
                    if tag in meta.tags:
                        metadata[tag], encoding = fix_charset_unicode(
                            gettag(tag, meta.tags), encoding)
                        if encoding and encoding != 'utf-8':
                            meta.tags[tag][0] = metadata[tag]
                            recoded = True
                            log.info("%s => %s (%s) [%s]" %
                                     (tag, metadata[tag], type(metadata[tag]),
                                      encoding))
    ...
    def get_actions(self, file, metadata, post):
        ...
        if fmt:
            item = {'file_id': file.id,
                    'path': g.fs.web(path),
                    'duration': int(file.metainfo['length']),
            ...
            return [
                {'action': 'add',
                 'class': 'add_',
                 'click': "add_to_playlist(%s);" % (item)},
                {'action': 'play',
                 'class': 'play_',
                 'click': "play_at_playlist(%s);" % (item)},
            ]
        else:
            return []
