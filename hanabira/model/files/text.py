...
class TextFile(Filetype):
    ...
    def get_thumb(self, filepath, text, lexer):
        imgfrm = ImageFormatter(line_pad=1,
                                font_size=8,
                                line_numbers=False,
    ...
    def process_metadata(self, metadata, file):
        return _("%s, %.2f KB, %s lines") % (metadata['type'], (
            file.size / 1024.0), metadata['lines'])

    def onclick_handler(self, file, metadata, post):
        return u"open_url('%s', '_blank')" % (url('util_text_view',
                                                  file_id=file.id,
                                                  post_id=post.id))
    ...
    def get_actions(self, file, metadata, post):
        return [
            {'action': 'view',
             'class': 'view_',
             'url': url('util_text_view',
                        file_id=file.id,
                        post_id=post.id)},
            {'action': 'edit',
             'class': 'edit_',
             'url': url('util_text_edit',
                        file_id=file.id,
                        post_id=post.id)},
        ]
