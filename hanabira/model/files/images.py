...
class ImageFile(Filetype):
    ...
    def preprocess(self, tf, ext, board):
        ...
        if board:
            if board.file_max_res and board.file_max_res < resolution:
                return (
                    tf, [],
                    ['image resolution (%s pixels) is too big' % resolution])
            if board.file_min_res > resolution:
                return (
                    tf, [],
                    ['image resolution (%s pixels) is too small' % resolution])
        # fingerprinting
        return (tf, [], None)
    ...
    def process_metadata(self, metadata, file):
        return u"%s, %.2f KB, %s×%s" % (metadata['type'], (file.size / 1024.0),
                                        metadata['width'], metadata['height'])

    def onclick_handler(self, file, metadata, post):
        return u"expand_image(event, %s, %s)" % (metadata['width'],
                                                 metadata['height'])
    ...
    def get_actions(self, file, metadata, post):
        ...
        return [
            {'action': 'edit',
             'class': 'edit_',
             'url': url('util_image_edit',
                        file_id=file.id,
                        post_id=post.id)},
            {'action': 'Find source with google',
             'class': 'search_google',
             'url': "http://www.google.com/searchbyimage?image_url=" + imgurl,
             'new_tab': True},
            {'action': 'Find source with iqdb',
             'class': 'search_iqdb',
             'url': "http://iqdb.org/?url=" + imgurl,
             'new_tab': True},
        ]
