...
def archive_zip(filepath):
    ...
    return list(map(lambda x: " ".join(x[3:]), filter(
        lambda x: len(x) > 3, map(lambda x: x.split(), r.split('\n')))[2:]))
...
def archive_7z(filepath):
    ...
    #return files
    return list(map(lambda x: " ".join(x[5:]), filter(
        lambda x: len(x) > 5, map(lambda x: x.split(), r.split('\n')))[2:-1]))
...
class ArchiveFile(Filetype):
    ...
    def process_metadata(self, metadata, file):
        if metadata is None:
            metadata = {}
        return _("%s, %.2f KB, %s files") % (metadata.get('type', 'unknown'), (
            file.size / 1024.0), metadata.get('files_count', 0))

    def onclick_handler(self, file, metadata, post):
        return u"open_url('%s', '_blank')" % (url('util_archive_view',
                                                  file_id=file.id,
                                                  post_id=post.id))
