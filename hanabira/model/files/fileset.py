...
class FileSet(object):
    ...
    def add_from_fobj(self, fobj, rating='unrated'):
        ...
        try:
            ext = g.extensions.get_ext(filename,
                                       allowed_filetypes=self.filetypes)
            if ext:
                tf = g.fs.store_temp_file(fobj=fobj,
                                          ext=ext.ext,
                                          rating=rating)
                if tf.get_fingerprint() in self.fingerprints:
                    ...
                    if self.board.file_min_size <= tf.size and tf.size <= self.board.file_max_size:
                        ...
                        self.errors.append(_('%s: file (%.2f kb) is too big') %
                                           (filename, tf.size / 1024.0))
                    elif self.board.file_min_size > tf.size:
                        self.errors.append(_('%s: file (%.2f kb) is too small')
                                           % (filename, tf.size / 1024.0))
            else:
    ...
    def add_from_data(self, data, filename, binary=False):
        try:
            ext = g.extensions.get_ext(filename,
                                       allowed_filetypes=self.filetypes)
            if ext:
                tf = g.fs.tf_from_data(data=data,
                                       ext=ext.ext,
                                       filename=filename,
                                       binary=binary)
                ...
                if fp in self.fingerprints:
                    ...
                    if self.board.file_min_size <= tf.size and tf.size <= self.board.file_max_size:
                        ...
                        self.errors.append(_('%s: file is too small') %
                                           filename)
            else:
                self.errors.append(_('%s: unknown file type') % filename)
        except Exception as e:
    ...
    def add_from_image(self, image, filename, **kw):
        try:
            ...
            if ext:
                tf = g.fs.tf_from_image(image=image,
                                        ext=ext.ext,
                                        filename=filename,
                                        **kw)
                fp = tf.get_fingerprint()
    ...
    def add_file(self, tf, ext):
        f2 = Fingerprint.get_file(type=ext.ext,
                                  quantifier=tf.size,
                                  fingerprint=tf.sha256)
        if f2:
            ...
            if errors:
                for err in errors:
                    self.errors.append(_('%s: %s') %
                                       (tf.filename_original, err))
                return None
            ...
            if fpl:
                for fp in fpl:
                    f2 = Fingerprint.get_file(type=fp[0],
                                              quantifier=fp[1],
                                              fingerprint=fp[2])
                    if f2:
            ...
            if not fp1 in fpl:
                fpl.append(fp1)
            file = File(ext=g.extensions.exts[tf2.ext],
                        temp_file=tf2,
                        keep_filename=self.keep_filenames)
            ...
            if file.file_id:
                ...
                self.errors.append(_('%s: could not be processed') %
                                   tf.filename_original)
        return None
