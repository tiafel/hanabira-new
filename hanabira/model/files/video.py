...
class VideoFile(Filetype):
    ...
    def process(self, file, thumb_res, fileset):
        ...
        r = popen(
            'ffmpeg -ss 0.5 -i "{0}" -vframes 1 -s {1}x{2} -f image2 {3}'.format(
                file.temp_file.path, th_w, th_h, thumb_path))
    ...
    def process_metadata(self, metadata, file):
        return "Video {0[Image Size]} x {0[Duration]}; {0[File Size]}".format(
            metadata)
