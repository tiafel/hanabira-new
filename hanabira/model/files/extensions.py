...
class Extension(meta.Declarative):
    ...
    filetype_id = Column(Integer,
                         ForeignKey('filetypes.filetype_id'),
                         nullable=False)
        ...
        if isinstance(fobj, cgi.FieldStorage):
            ...
            if ext in self.exts:
                if self.exts[ext].filetype.type in allowed_filetypes:
                    ...
                    f2 = File.query.filter(File.ext_id == f.ext_id).filter(
                        File.fingerprint == f.fingerprint).first()
                    if f2:
                        return f2
                    else:
                        return f
