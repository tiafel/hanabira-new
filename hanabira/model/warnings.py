...
class WarningRecord(meta.Declarative):
    ...
    def get_for_session(cls, sess):
        ...
        q = cls.query.filter(or_(cls.session_id == sess.id, cls.subnet.in_(
            [sub1, sub2])))
...
reasons_list = [
    # 2.2
    (20201,
     u'2.2 Воспрепятствование обсуждению темы: сажа/скрыл/опхуй/тредненужен',
    ...
    (20203,
     u'2.2 Воспрепятствование обсуждению темы: бессмысленные сообщения/вайп',
    ...
    (20323, u'2.3 Нарушение специальных правил треда: ненормативные выражения',
     u'— п.2.3, в данном треде запрещена ненормативная лексика;', u'маты'),
...
def generate_post_warn_message(reasons,
                               post,
                               thread,
                               board,
                               deleted=False,
    ...
    if token and not token[0] in ['premod', 'bypass_premod', 'none']:
        scope = u""
        if token[1][0] == 'board':
            scope = u" в разделе <a href='/{0}/index.xhtml'>>>/{0}/</a>".format(
                token[1][1])
        elif token[1][0] == 'thread':
            scope = u" в треде <a href='{2}' onmouseover=\"ShowRefPost(event,'{0}', {1}, {1});\">>>/{0}/{1}</a>".format(
                board.board,
                thread.display_id,
                url('thread',
                    board=board.board,
        ...
        if token[2] != -1:
            nt = datetime.datetime.now().replace(
                microsecond=0) + datetime.timedelta(minutes=token[2])
            expires = u" до {0} ({1} минут).".format(nt, token[2])
        msg.append(message_token + message_token_dict[token[0]] + scope +
                   expires)
    elif not deleted:
        msg.append(
            u"настоятельно просим в дальнейшем при написании сообщений учитывать данные замечания и не нарушать ToS.")
    msg.append(message_end)
    msg = u"<br/>\n".join(msg)
    thread_url = url('thread', board=board.board, thread_id=thread.display_id)
    post_link = "<a href='{2}#i{1}' onmouseover=\"ShowRefPost(event,'{0}', {3}, {1});\">>>/{0}/{1}</a>".format(
        board.board, post.display_id, thread_url, thread.display_id)
...
def format_token_public(tokens):
    if tokens[0][0] == 'forbid_post':
        ...
        return u'Вам запрещено писать сообщения в данном треде, ограничение {time}.'.format(
            time=dur)
    else:
