...
class Channel(meta.Declarative):
    ...
    def __repr__(self):
        return "<Channel %s @ %s>" % (self.title.encode('utf-8'),
...
class Section(meta.Declarative):
    ...
    def __repr__(self):
        return "<Section(%s)[%s][%s]>" % (str(self.title), self.section_id,
                                          self.chan_id)
...
class Board(meta.Declarative):
    ...
    allowed_filetypes = Column(
        PickleType(pickler=pickle),
        nullable=False,
        default=default_allowed_ft)
    allowed_new_files = Column(
        PickleType(pickler=pickle),
    ...
    # Mappings
    __mapper_args__ = {'polymorphic_on': indexing_type,
                       'polymorphic_identity': 'board-board'}
    ...
    def check_permissions(self, type, admin, chan=None):
        ...
        if self.__getattribute__(restrict_type):
            ...
            return not chan or self.chan.id == chan.id or (
                admin and admin.has_permission('boards'))

    def check_require_captcha(self):
        if not self.require_captcha:
            return False
        if session.get('admin', None) and session['admin'].has_permission(
                'no_captcha', self.id):
            return False
        if session['captcha'].get_complexity(
                self.board) > g.settings.captcha.optional_complexity:
            return True
        if not session[
                'enforced_captcha_complexity'] and g.settings.captcha.post_threshold != -1 and g.settings.captcha.post_threshold <= (
                    session.posts_visible):
            return False
        else:
            return True
    ...
    def check_posting_limits(self, post, _post_limits):
        ...
        if post.op == True:
            qty = Post.query.filter(Post.op == True).filter(
                Post.session_id == session.id).filter(
                    Post.date >= hour_ago).filter(
                        Post.display_id != None).count()
            if qty >= self.threads_per_hour:
                complexity = ecc + (g.settings.captcha.new_thread_penalty *
        ...
        if post.op == False:
            posts = Post.query.filter(Post.thread_id == post.thread_id).filter(
                Post.display_id != None).filter(
                    Post.date >=
                    hour_ago).order_by(Post.display_id.desc()).all()
            consecutive = 0
            while posts:
                ppost = posts.pop(0)
            ...
            if consecutive >= self.replies_per_hour:
                complexity = ecc + (g.settings.captcha.reply_penalty *
    ...
    def thread_filters(self,
        ...
        if not query:
            query = Thread.query
        query = query.filter(Thread.board_id == self.id).filter(
            Thread.display_id != None)
            ...
            if visible_posts:
                ...
                if visible_threads:
                    query = query.filter(or_(
                        Thread.thread_id.in_(visible_threads), Thread.invisible
                        == False))
                else:
    ...
    def get_thread_count(self,
                         ...
                         sticky=None):
        if archive:
            f = self.thread_filters(archive=True,
                                    see_invisible=False,
                                    sticky=False)
        else:
            f = self.thread_filters(archive=archive,
                                    hidden=hidden,
    ...
    def get_pages(self, admin, hidden=None, archive=False, visible_posts=None):
        if self.check_permissions('read', admin):
            see_invisible = (admin and admin.has_permission('see_invisible',
                                                            self.board_id))
            q = self.get_thread_count(hidden=hidden,
                                      archive=archive,
                                      visible_posts=visible_posts,
                                      see_invisible=see_invisible,
                                      sticky=None)
    ...
    def get_thread(self,
                   ...
                   get_posts=True):
        if self.check_permissions('read', admin):
            thread = Thread.query.filter(Thread.board_id == self.id).filter(
                Thread.display_id == thread_id).first()
            view_deleted = admin and admin.has_permission('view_deleted',
                                                          self.id)
            if not thread and (deleted or view_deleted):
                thread = DeletedThread.query.filter(
                    DeletedThread.board_id == self.id).filter(
                        DeletedThread.display_id == thread_id).first()
            if thread:
                if fetch:
                    thread.fetch_posts(admin, visible_posts, deleted, after,
                                       get_posts)
                return thread
            else:
                return None
    ...
    def get_thread_simple(self, display_id):
        thread = Thread.query.filter(Thread.board_id == self.id).filter(
            Thread.display_id == display_id).first()
        return thread

    def get_thread_by_id(self, thread_id, admin):
        if self.check_permissions('read', admin):
            thread = Thread.query.filter(Thread.board_id == self.id).filter(
                Thread.thread_id == thread_id).first()
            if thread:
                thread.op = Post.post_filters(Post.query.filter(
                    Post.post_id == thread.op_id)).first()
                return thread

    def get_post(self, post_id, try_deleted=False):
        post = Post.query.options(eagerload('thread')).join(Thread).filter(
            Post.display_id == post_id).filter(
                Thread.board_id == self.board_id).first()
        if not post and try_deleted:
            post = DeletedPost.query.options(eagerload('thread')).join(
                Thread).filter(DeletedPost.display_id == post_id).filter(
                    Thread.board_id == self.board_id).first()
        return post

    def last_bump(self, thread_id=None, display_id=None):
        if display_id:
            q = Post.query.options(eagerload('thread')).filter(
                Thread.id == display_id).filter(Thread.board_id == self.id)
        elif thread_id:
            q = Post.query.filter(Post.thread_id == thread_id)
        else:
            q = None
        if q:
            return q.filter(Post.sage == False).filter(
    ...
    def get_visible_threads(self, page=0, archive=False, hidden=None):
        ...
        threads_query = self.thread_filters(
            archive=archive, hidden=hidden,
            sticky=False).order_by(Thread.last_hit.desc())
        threads = threads_query.offset(offset).limit(
            self.threads_per_page).all()
        return threads

    def get_invisible_threads(self,
                              page=0,
                              archive=False,
                              hidden=None,
                              visible_posts=None):
    ...
    def get_all_threads(self, page=0, archive=False, hidden=None):
        ...
        threads_query = self.thread_filters(
            ...
            see_invisible=True,
            hidden=hidden,
            sticky=False).order_by(Thread.last_bumped.desc())
        threads = threads_query.offset(offset).limit(
            self.threads_per_page).all()
        return threads
    ...
    def get_archive_threads(self, page=0):
        offset = self.threads_per_page * page
        threads_query = self.thread_filters(
            archive=True, see_invisible=False,
            sticky=False).order_by(Thread.last_bumped.asc())
        threads = threads_query.offset(offset).limit(
            self.threads_per_page).all()
        if not threads:
            return []
        threads = Thread.populate_threads(threads,
                                          self,
                                          see_invisible=False,
    ...
    def get_threads(self,
        ...
        if self.check_permissions('read', admin):
            ...
            vis_threads = visible_posts and (
                visible_posts.get_threads(self.board_id) +
                list(visible_posts.get_board(self.id)['posts'].keys()))
            if (see_invisible is None and visible_posts is None) or (
                    not see_invisible and not vis_threads):
                threads = self.get_visible_threads(page=page,
                                                   archive=archive,
                                                   hidden=hidden)
            elif see_invisible:
                threads = self.get_all_threads(page=page,
                                               archive=archive,
                                               hidden=hidden)
            else:
                threads = self.get_invisible_threads(
                    page=page,
                    archive=archive,
                    hidden=hidden,
                    visible_posts=visible_posts)
            ...
            if not ignore_sticky and not archive and page == 0:
                sticky = self.thread_filters(
                    archive=None,
                    hidden=hidden,
                    sticky=True,
                    see_invisible=True).order_by(Thread.last_hit.desc()).all()
        ...
        if not password is None and ip and session_id:
            if thread_id:
                thread = Thread.query.filter(
                    Thread.board_id == self.board_id).filter(
                        Thread.display_id == thread_id).first()
            else:
                thread = Thread()
            now = datetime.datetime.now()
            ...
            if thread:
                if thread.id:
                    ...
                    if thread.locked and not admin:
                        ...
                        critical_errors.append(_('Thread %s is locked') %
                                               thread_id)
                    if critical_errors:
            ...
            log.info('empty_post(): not %s is None and %s and %s == False' %
                     (password, ip, session_id))
            return None
    ...
    def new_post(self, post_id, session, req, hold=False):
        ...
        if post_id:
            ...
            post = self.empty_post(thread_id=thread_id,
                                   password=password,
                                   admin=admin,
                                   ip=req.ip,
                                   geoip=req.country,
        ...
        if len(params.get('message', '')) > 60000:
            add_error(errors, 'message', _(
                'Message should not exceed 60000 chars (your is {0} chars)').format(
        ...
        post.date = now
        post.sage = thread.autosage or (not post.op and params.get('sage',
                                                                   False))
        if post.sage:
            post.sage = True
        ...
        if self.allow_files and not session.get_token('forbid_files',
                                                      thread=thread,
                                                      board=self):
        ...
        if not (post.message_raw or post.files):
            add_error(errors, 'message',
                      _('You should either write message or add file'))

        if post.op and self.require_thread_file and not post.files:
            add_error(errors, 'file',
                      _('You have to add at least one file for new thread'))
        if post.op and not (post.message_raw or post.subject):
            add_error(errors, 'subject',
                      _('You should either write subject or message'))
        ...
        if self.check_posting_limits(post, _post_limits):
            ...
            if session['captcha'].check_complexity(self, _post_limit[1]):
                if not session['captcha'].check(self, params, admin):
                    ...
                    session['captcha'].new(self.board,
                                           complexity=_post_limit[1])
            else:
            ...
            session['captcha'].new(
                self.board,
                complexity=g.settings.captcha.optional_complexity)
    ...
    def update_self(self):
        q = list(Thread.query.filter(Thread.board_id == self.board_id).values(
            func.sum(Thread.posts_count)))
        self.posts = q[0][0]
    ...
    def set_display_id(self, thread, post):
        with locked(self.index_lock):
            thread_idx, post_idx = list(Board.query.filter(
                Board.board_id == self.id).values(Board.thread_index,
                                                  Board.post_index))[0]
    ...
    def delete_posts(self,
        ...
        for thread_id in threads:
            ...
            if thread:
                log.debug("Delete from thread %s:%s %s" % (
                    thread.thread_id, thread.display_id, threads[thread_id]))
                if thread.archived and not (
                        admin and admin.has_permission('manage_archive')):
                    return _("Can't delete posts from archive.")
                res = thread.delete_posts(board=self,
    ...
    def stats_posts(self):
        idx = meta.Session.query(Board.post_index).filter(
            Board.board_id == self.id).first()[0]
        return idx
    ...
    def archive_threads(self):
        ...
        threads = self.thread_filters(see_invisible=True).filter(
            Thread.last_hit < date_min).filter(Thread.invisible == True).all()
        log.info('Found %s invisible threads updated before %s' %
                 (len(threads), date_min))
        inv_del = 0
        inv_arch = 0
        for thread in threads:
            ...
            to_remove -= 1
        log.info(
            "Deleted {0} and archived {1} invisible threads, need to remove {2} more.".format(
                inv_del, inv_arch, to_remove))

        threads = self.thread_filters().filter(
            Thread.last_hit < date_min).filter(
                Thread.sticky == False).order_by(Thread.last_hit).all()
        log.info("Found %s threads updated before %s" %
                 (len(threads), date_min))
        while to_remove and len(threads):
            thread = threads.pop(0)
            days = (now - thread.last_hit).days
            log.info("Thread %s, posts %s, last_hit %s (%s days)" % (
                thread.display_id, thread.posts_count, thread.last_hit, days))
    ...
    def set_context_permissions(self, context, admin):
        if admin:
            ...
            context.can_see_invisible = admin.has_permission('see_invisible',
                                                             self.id)
            context.can_delete_posts = admin.has_permission('delete_posts',
                                                            self.id)
            context.can_edit_posts = admin.has_permission('edit_posts',
                                                          self.id)
        else:
    ...
    def __repr__(self):
        return "<Board(/%s/, %s, %s)>" % (self.board, self.indexing_type,
                                          self.post_index)
    ...
    def load(self):
        ...
        for chan in Channel.query.all():
            ...
            chan.sections = Section.query.filter(
                Section.chan_id == chan.id).order_by(Section.index.asc()).all()
            chan.boards = {}
            chan.board_ids = {}
            for section in chan.sections:
                section.boards = Board.query.options(eagerload(
                    'section')).filter(
                        Board.section_id == section.section_id).order_by(
                            Board.section_index.asc()).all()
                for board in section.boards:
                    chan.boards[board.board] = chan.board_ids[
                        board.id] = self.boards[board.board] = self.board_ids[
                            board.id] = board
                    board.chan = chan
                    ...
                    if os.path.exists(self.fs.local(bp)):
                        banners = [self.fs.web(os.path.join(bp, x))
                                   for x in os.listdir(self.fs.local(bp))]
    ...
    def get_banner(self, board, session):
        ...
        if bs == 'different':
            if board.chan.id in self.chan_banners:
                bname = random.choice(list(self.chan_banners[
                    board.chan.id].keys()))
