...
utf_trash_set = set(
    [8206, 8207, 8238, 8234, 8235, 8237, 8236, 8203, 8204, 8205])
...
def popen(command, raise_errors=False):
    if raise_errors:
        ...
        e = sp.stderr.read()
        if e:
            raise Exception(_("%s returned error:\n%s") %
                            (command.split(' ')[0], e))
        r = sp.stdout.read()
    else:
...
def whois(ip, ww=None, html=None):
    ...
    for line in r.split('\n'):
        if line.find('inetnum') == 0:
            ...
            if range == '0.0.0.0 - 255.255.255.255' and not ww:
                ...
                res.append('%s%s' %
                           (range, mask and ' (mask %s)' % mask or ''))
        if line.find('descr') == 0 or line.find('address') == 0 or line.find(
                'netname') == 0:
    ...
    if rereq:
        ...
        res = '\n'.join(res)
        if html:
            return '%s\n<input id="subnet" type="hidden" value="%s"/>' % (
                res, range)
        else:
            return res
...
def log_caller():
    s = inspect.stack()
    log.info("%s called from (%s):%s from function %s()" %
             (s[1][3], s[2][1], s[2][2], s[2][3]))
...
ib_url_re = re.compile(
    r'https?://([^/]+)/([^/]+)/((\d+)|res/(\d+)|\w+)(\.x?html)?(#i?(\d+))?')
