...
def filepan(files, this_id):
    str = '''onmousedown="get_filepan(event, %s, '%s''' % (this_id,
                                                           files[0].id)
    for f in files[1:]:
        str = '%s_%s' % (str, f.id)
    return '''%s')"''' % str
...
def userpan(post, thread):
    ...
    return ''' onmousedown="get_userpan(event, {0.id}, {1.id}, '{0.session_id}', {2:g}, {0.deleted:g})"'''.format(
        post, thread, invis)


def geoip(geoip):
    return '''<img class="geoicon" src="/images/geo/%s.png" alt="%s" title="%s" />''' % (
        geoip.lower(), geoip, g.country_codes.get(geoip, ''))
...
def expandName(name):
    ...
    if name:
        parts = name.split('.')
        if parts and utils.isNumber(parts[-2].replace(
                's', '')):  #TODO: is the second condition really needed?
            ext = parts[-1].lower()
            prefix = name[0:4]
...
def static_domain(filename):
    url = "http://{0}{1}{2}".format(c.channel.host, g.settings.path.static_web,
                                    filename)
    return url
...
def static_versioned(filename):
    ...
    return "%s%s-%s.%s" % (g.settings.path.static_web, name,
                           g.settings.dist.version, ext)
...
def show_time(time, ctxt, timestamp=False):
    ...
    tf = str(ctxt.mini and g.settings.chan.mini_timeformat or
             g.settings.chan.timeformat)
    if 'ru' in ctxt.lang:
...
def button(klass,
    ...
    if onclick: str = '%s onclick="%s"' % (str, onclick)
        ...
        str += ' onclick="' + "window.open(this.href,'_blank');return false;" + '" '
...
def make_ip_hash(thread_id, session_id):
    md5 = hashlib.md5(str(thread_id).encode('utf-8') + session_id.encode(
        'utf-8')).hexdigest().upper()
    marks = []
    for color in [md5[x * 6:x * 6 + 6] for x in range(5)]:
        marks.append(
            "<span class='ipmark' style='background: #%s;'>&nbsp;</span>" %
            color)
    return ''.join(marks)
