...
class PTORef(PTO):
    ...
    def build_etree(self, wm, parent):
        ...
        href = url(
            ...
            thread_id=post.thread.display_id) + '#i' + str(post.display_id)
        reflink = (
            'a', [('href', href),
                  ('onmouseover', "ShowRefPost(event,'%s', %d, %d)" %
                   (refboard.board, post.thread.display_id, post.display_id)),
                  ('onclick', "Highlight(event, '%d')" % post.display_id)])
...
class WakabaMessage(object):
    ...
    def push_block(self):
        ...
        if self.blocks and (self.current_block_type in ('ref', 'text') and
            ...
            self.blocks.append(
                [self.current_block_type, self.current_block_lines])
    ...
    def walk_blocks(self):
        ...
        for block_type, block_lines in self.blocks:
            ...
            if block_type == 'code':
                self.current_container = etree.SubElement(
                    self.current_container, "code")
            elif block_type == 'spoiler':
                self.current_container = etree.SubElement(
                    self.current_container, "div")
        ...
        if self.total_lines_count and self.message_short is None:
            if self.total_lines_count + 1 > lines_limit or \
               self.total_lines_len + len(line) > chars_limit:
                self.message_short = etree.tounicode(self.xml,
                                                     pretty_print=False)
