...
class Captcha(object):
    ...
    def new(self, board, complexity=None):
        ...
        self.captchas[board] = g.captchas.generate(language=self.language,
    ...
    def get(self, board):
        ...
        if type(self.complexity) != dict:
            self.complexity = {}
        if not board in self.captchas or time.time() - self.time[
                board] > 300000:
            self.new(board)
...
class Captchas(object):
    ...
    def __init__(self, g):
        ...
        self.words = {}
        for fn in [x for x in os.listdir(os.path.join(g.root, 'config'))
                   if 'captcha' in x]:
            lang = fn.split('.')[1]
            fp = os.path.join(g.root, 'config', fn)
            self.words[lang] = list(filter(bool, [x.lower().strip(
            ) for x in open(fp, 'r+b').read().decode('utf-8').split("\n")]))
            log.info("%s words in %s captcha dict" %
                     (len(self.words[lang]), lang))
