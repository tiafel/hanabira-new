...
from pylons import (tmpl_context as c, app_globals as g, cache, config,
                    request, response, session, url)
...
def result(r, **kw):
    return dict(result=r, error=None, opts=kw)
...
def error(c, m="", a="", **kw):
    ...
    return dict(result=None,
                error=dict(code=et[0],
                           alias=et[1],
                           message=_(et[2]), ),
                opts=kw)


def ExportJSON(threads=None,
               events=None,
               admin=None,
               board=None,
               page=None,
    ...
    if board:
        res['boards'][board.board] = {'threads': [],
                                      'capabilities': board.get_capabilities()}
    ...
    for thread in threads:
        ...
        if not b.board in res['boards']:
            res['boards'][b.board] = {'threads': [],
                                      'capabilities': b.get_capabilities()}
        res['boards'][b.board]['threads'].append(thread.export_dict(
            admin=admin))
    response.headers['Content-Type'] = 'application/json'
