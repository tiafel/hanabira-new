...
class NewFile(object):
    ...
    def __init__(self, id, post_id, filename, filetype, salt):
        self.id = id
        self.key = hashlib.sha256(str(long(time.time() * 10**7)) +
                                  hashlib.sha256(salt).hexdigest()).hexdigest()
...
class NewFiles(object):
    ...
    def get(self, id, key):
        if len(self.newfiles) > id and self.newfiles[
                id].valid and self.newfiles[id].key == key:
    ...
    def new(self, post_id, filename, filetype):
        ...
        self.newfiles.append(NewFile(fid, post_id, filename, filetype, str(
            self.settings.security.hash.salt)))
