...
class VisiblePosts(object):
    ...
    def get_posts(self, thread):
        return self.get_board(thread.board_id)['posts'].get(thread.thread_id,
                                                            {}).keys()
    ...
    def add_post(self, post, thread):
        ...
        self.cont[thread.board_id]['posts'][thread.thread_id][
            post.post_id] = bump_date
    ...
    def has_thread(self, thread):
        return thread.id in self.cont.get(thread.board_id,
                                          {'threads': []})['threads']
    ...
    def get_bumps(self, board_id):
        ...
        for tid in posts:
            ...
            thread_bumps = sorted(filter(lambda x: thread_posts[x],
                                         thread_posts))
            if thread_bumps:
...
def ensure_visible(sess):
    if not sess: return
    if not 'visible_posts' in sess or sess[
