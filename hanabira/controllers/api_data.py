...
class ApiDataController(BaseController):
    ...
    def thread_all(self, thread, board):
        ...
        q = Post.post_filters(
            ...
            see_invisible=session.get_token(
                'see_invisible',
                board_id=board.id)).order_by(Post.display_id.asc())
        thread.posts = q.all()
        return ThreadAPIView(thread, board)
    ...
    def thread_new(self, thread, board):
        ...
        q = Post.post_filters(
            ...
            visible_posts=session['visible_posts'].get_posts(thread),
            see_invisible=session.get_token(
                'see_invisible', board_id=board.id)).filter(
                    Post.display_id >
                    last_post).order_by(Post.display_id.asc())
        thread.posts = q.all()
    ...
    def thread_last(self, thread, board):
        ...
        q = Post.post_filters(
            thread=thread,
            visible_posts=session['visible_posts'].get_posts(thread),
            see_invisible=(c.admin and c.admin.has_permission('see_invisible',
                                                              board.board_id)))
        if last_post:
    ...
    def thread_expand(self,
        ...
        if board and c.board.check_permissions('read', c.admin, c.channel):
            thread = board.get_thread(thread_id,
                                      admin=c.admin,
                                      visible_posts=ensure_visible(session),
                                      after=last_post)
            if thread:
                c.reply = int(reply)
                c.expanded = not last_post
                board.set_context_permissions(c, c.admin)
                return render('/view/thread_expansion.mako', {'thread': thread,
                                                              'board': board})
            else:
