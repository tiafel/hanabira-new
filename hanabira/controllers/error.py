...
class ErrorController(BaseController):
    ...
    def document(self):
        ...
        c.http_error = code = request.environ.get(
            'pylons.original_response').status_int
        ...
        if b in ['src', 'thumb']:
            ...
            if c.board and c.board.check_permissions('read', c.admin,
                                                     c.channel):
                c.pageinfo.setboard(c.board)
            elif b and not re.search(
                    re.compile(u'^[\_\s]*([Дд][OОоo0][Бб6][РPрp]|good|dobr)',
                               re.I), b):
                c.pageinfo.title = u'/%s/' % b
        if code == 404:
            c._404image = g._404images and '/images/404/' + random.choice(list(
                g._404images)) or u""
