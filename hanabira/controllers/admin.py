...
class AdminController(BaseController):
    ...
    def check_scope(self, scopes, scope):
        ...
        if scope in scopes:
            return True
        raise PermissionException("Scope {0} is missing in {1}".format(scope,
                                                                       scopes))

    def log_admin_action(self, event, **kw):
        event(ip=ipToInt(request.ip),
    ...
    def admins_edit(self, admin_id):
        c.pageinfo.subtitle = _('Admins')
        admin = Admin.query.options(eagerload('permissions')).filter(
    ...
    def admins_permission_add(self, admin_id):
        ...
        admin.add_permission(
            request.POST.get('permission'), request.POST.get('scope'))
    ...
    def help_edit(self, help_id):
        ...
        article = HelpArticle.query.filter(
            HelpArticle.help_id == help_id).first()
        if not article:
            return abort(404)
        c.article = article
    ...
    def boards_edit(self, board_id):
        c.pageinfo.subtitle = _('Boards')
        c.board = board = Board.query.filter(
            Board.board_id == board_id).first()
        form = BoardForm(board, filetypes=g.extensions.types.keys())
    ...
    def sections_edit(self, section_id):
        ...
        c.section = section = Section.query.filter(
    ...
    def files(self, filter_id, page, scopes=None):
        ...
        filters = {'type': (Filetype, 'list', g.extensions.types.keys()),
                   'rating':
                   (File, 'list',
                    ['unrated', 'sfw', 'r-15', 'r-18', 'r-18g', 'illegal'])}
        sort = {'file_id': None,
                'type': Filetype,
                'rating': None,
                'date_added': None}
    ...
    def files_by_id(self, file_id, scopes=None):
        ...
        if f > 0:
            ...
            base    = FilterBase(model=File, filters=filters, sort={}, query='get_query_with_ft',\
                                 default_per_page=1)
            filter_id = g.filters.save(Filter(base=base,
    ...
    def file_ajax(self, file_id, scopes=None):
        c.files = File.query.join(Filetype).filter(File.file_id.in_(
            file_id.split('_'))).all()
        if len(c.files):
    ...
    def files_edit(self):
        if request.POST:
            f = File.query.filter(
                File.file_id == request.POST.get('file_id', None)).first()
            if not f:
                return abort(404)
    ...
    def restrictions(self, filter_id, page):
        ...
        page = page and int(page) or 0
        filters = {'type': (Restriction, 'list', restriction_types),
                   'effect': (Restriction, 'list', effects),
                   'expired': (Restriction, 'list', ['0', '1'])}
        sort = {'restriction_id': None,
                'type': None,
                'date_added': None,
                'value': None,
                'effect': None,
        ...
        if req:
            ...
            if type in g.restrictions.classes and value and effect:
                ...
                if r.validate():
                    meta.Session.add(r)
                    meta.Session.commit()
                    self.log_admin_action(ModRestrictionActionLog,
                                          restriction=r,
                                          action=_("added"))
                    meta.Session.commit()
                    g.restrictions.load()
    ...
    def restrictions_expire(self, restriction_id):
        r = Restriction.query.filter(
            Restriction.restriction_id == restriction_id).first()
        if not r:
            return abort(404)
        r.expired = not r.expired
        self.log_admin_action(ModRestrictionActionLog,
                              restriction=r,
                              action=(r.expired and _("disabled") or
                                      _("enabled")))
        meta.Session.commit()
        g.restrictions.load()
        reload_nodes()
    ...
    def featured(self):
        if request.POST:
            ...
            thread = Thread.query.filter(Thread.board_id == board.id).filter(
                Thread.display_id == thread_display_id).one()
            post = Post.query.filter(
                Post.display_id == post_display_id).filter(
                    Post.thread_id == thread.thread_id).one()
    ...
    def featured_add(self, post_id):
        ...
        if request.POST:
            ...
            f = Featured.new(post_id=post.id,
                             file_id=(show_file and post.files[0].id),
    ...
    def get_post(self, post_id):
        ...
        c.reply = False
        c.thread = thread = c.post.thread or Thread.query.filter(
            Thread.id == post.thread_id).first()
        c.session = g.sessions.load_by_id(post.session_id)
        c.events = BaseEventLog.query.filter(
            BaseEventLog.post_id ==
            post_id).order_by(BaseEventLog.date.desc()).all()
        self.log_admin_action(ModViewPost, post=post, commit=True)
        if not thread:
            c.thread = thread = DeletedThread.query.filter(
                DeletedThread.id == post.thread_id).first()
            c.pageinfo.description = _("This post's thread is deleted.")
        elif post.deleted:
            c.pageinfo.description = _("This post is deleted.")
        if not thread:
            return DataView(_("Thread id {0} does not exist.").format(
                post.thread_id))
        if post.op:
    ...
    def post_edit(self, post_id, format=None, scopes=None):
        ...
        if not post:
            return abort(404)
        if request.POST:
            post.add_revision(name=request.POST.get('name'),
    ...
    def get_thread(self, thread_id):
        thread, op = meta.Session.query(Thread, Post).join(
            (Post, Thread.op_id == Post.post_id)).filter(
                Thread.id == thread_id).first() or (None, None)
        if not thread:
            thread, op = meta.Session.query(DeletedThread, DeletedPost).join(
                (DeletedPost,
                 DeletedThread.op_id == DeletedPost.post_id)).filter(
                     DeletedThread.id == thread_id).first() or (None, None)
        ...
        if thread:
            ...
            c.events = BaseEventLog.query.filter(
                BaseEventLog.post_id ==
                op.post_id).order_by(BaseEventLog.date.desc()).all()
        else:
    ...
    def session(self, session_id):
        ...
        if request.POST:
            if 'premod_reason' in request.POST:
                ...
                wr = WarningRecord('premod',
                                   session_id,
                                   c.session.last_ip,
                                   reason=reason)
                c.session.add_token('premod', ('global', None),
                                    -1,
                                    admin=session.admin,
                                    reason_text=reason)
                c.session.save()
                self.log_admin_action(ModSessionActionLog,
                                      target_session_id=session_id,
                                      action="added global premod",
                                      commit=True)
                ...
                self.log_admin_action(
                    ModSessionActionLog,
                    target_session_id=session_id,
                    action="removed " + request.POST['token_data'],
                    commit=True)
        else:
            self.log_admin_action(ModViewSession,
                                  target_session_id=session_id,
                                  commit=True)
        c.events = BaseEventLog.query.filter(
            BaseEventLog.target_session_id ==
            session_id).order_by(BaseEventLog.date.desc()).limit(20).all()
        c.referers = Referer.query.filter(
            Referer.session_id ==
            session_id).order_by(Referer.date.desc()).limit(20).all()
    ...
    def session_recount(self, session_id):
        ...
        deleted_posts = DeletedPost.query.filter(
            DeletedPost.session_id == session_id).count()
        inv_posts = Post.query.filter(Post.session_id == session_id).filter(
            Post.invisible == True).count()
        sess.posts_count = posts + deleted_posts
        sess.posts_invisible = inv_posts
        sess.posts_deleted = deleted_posts

        threads = Post.query.filter(Post.session_id == session_id).filter(
            Post.op == True).count()
        deleted_threads = DeletedPost.query.filter(
            DeletedPost.session_id == session_id).filter(
                DeletedPost.op == True).count()
    ...
    def thread_merge(self, subj, dest):
        if dest.find('_') > -1:
            ...
            if board:
                dest_thread = Thread.query.filter(
                    Thread.display_id == display).filter(
                        Thread.board_id == board.id).first()
                if dest_thread: dest = dest_thread.id
                else:
                    return "alert('Here is no thread with display_id %s.')" % (
                        display)
            else:
                return abort(404)
        else:
        ...
        posts = Post.post_filters(Post.query.filter(Post.thread_id ==
                                                    subj)).all()
        for post in posts:
            post.thread_id = dest
            post.op = False
    ...
    def get_file_post(self, file_id):
        ...
        if not f:
            return abort(404)
        c.posts = f.posts
        c.pageinfo.description = u'Posts with file {0.filename} ({0.file_id})'.format(
            f)
    ...
    def userposts(self):
        ...
        if do == 'delete':
            ...
            try:
                l = 'Deleting threads: '
                for thread in Thread.query.filter(Thread.id.in_(
                        thread_ids)).all():
        ...
        if do == 'delform':
            ...
            for tid in threads:
                ...
                thread.delete_posts(thread.board, threads[tid], None, c.admin,
                                    request, session)
            meta.Session.commit()
            return HTMLView(
                '<html><script>javascript:history.go(-1)</script></html>')
        ...
        q = post_cls.query.options(eagerload('thread')).join(Thread).filter(
            post_cls.display_id != None)
        ...
        q = {
            "ev": lambda: q,
            "ds":
            lambda: q.filter(post_cls.date > datetime.now() - timedelta(hours=float(req['qty']) * 24)),
            "hr":
            lambda: q.filter(post_cls.date > datetime.now() - timedelta(hours=float(req['qty']))),
            "ps":
            lambda: q.filter(post_cls.display_id > board.post_index - int(req['qty']))
        }[req['limit']]()
        q = {
            "oa": lambda: q,
            "tt": lambda: q.filter(post_cls.thread_id == post.thread_id),
            "tb":
            lambda: q.filter(Thread.board_id == board.id).filter(Thread.archived == False),
        ...
        if do == 'stat':
            ...
            if ops == 'ex':
                ...
                if ops == 'in':
                    posts = q.filter(
                        Post.op ==
                        False).filter(not_(Thread.id.in_(thread_ids))).all()
                    post_count = q.filter(Post.op ==
                                          False).count() + threads_to_delete
                    posts_to_delete = len(posts)
                    post_ids = map(lambda x: x.id, posts)
            if threads_to_delete and loc != 'tt':
                report = '%s posts: %s threads (totally %s posts) and %s in other threads' % (
                    post_count, threads_to_delete, from_own_threads,
                    posts_to_delete)
            elif loc == 'tt':
    ...
    def list_lastposts(self):
        req = request.POST
        q = Post.query.options(eagerload('thread')).join(Thread).filter(
            Post.display_id != None).filter(
                Post.date >
                datetime.now() - timedelta(hours=float(req['time'])))
        ...
        if not c.admin.has_permission('see_invisible'):
            q = q.filter(Post.invisible == False)
        elif req.get('inv'):
