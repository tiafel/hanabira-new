...
class ApiAdminController(BaseController):
    ...
    def delete_post(self, post, thread, board):
        ...
        if post.op:
            ...
            self.log_admin_action(ModThreadDeleteLog,
                                  thread=thread,
                                  commit=True,
                                  reason="")
        else:
            post.delete_self(deleter='mod')
            self.log_admin_action(ModPostDeleteLog,
                                  post=post,
                                  commit=True,
                                  reason="")
        return result_true
    ...
    def thread_set(self, thread, attr, value):
        ...
        if attr == 'archived':
            ...
            meta.Session.commit()
        elif attr in ['autosage', 'sticky', 'locked', 'op_moderation',
                      'censor_lim', 'censor_full']:
            now = datetime.now()
    ...
    def warnban_user(self, post, thread, board):
        ...
        if token_type != 'none':
            if not token_type in ['forbid_post', 'premod', 'bypass_premod',
                                  'forbid_name', 'forbid_name_subj',
                                  'forbid_files']:
            ...
            if token_type == 'forbid_name_subj':
                sess.add_token('forbid_name',
                               token_scope,
                               token_duration,
                               reason_post_id=post.id,
                               reasons=reasons,
                               admin=session.admin)
                sess.add_token('forbid_subj',
                               token_scope,
                               token_duration,
                               reason_post_id=post.id,
                               reasons=reasons,
            ...
            self.log_admin_action(ModSessionActionLog,
                                  ...
                                  action="added {0}:{1[0]}:{1[1]}".format(
                                      token_type, token_scope))

        # Send warning
        if reasons and send_warning:
            msg = generate_post_warn_message(reasons,
                                             post,
                                             thread,
                                             board,
                                             deleted=(add_action == 'del'),
                                             token=(token_type, token_scope,
                                                    token_duration))
