...
error_codes_list = [
    ...
    (40301, 'element_read_not_allowed',
     N_('Current session does not have access to specified element.')),
    (40302, 'board_read_not_allowed',
     N_('Current session does not have access to specified board.')),
    (40303, 'temporary_restricted_to_author',
     N_('This feature is temporarily restricted to post author and moderators.')
     ),
    (40310, 'admin_permission',
     N_('Your account does not have needed permission token.')),
    # 4035x - modify permission error
    (40351, 'element_modify_not_allowed',
     N_('Current session does not have permissions to edit specified element')
     ),

    # 400xx - bad params
    (40001, 'api_missing_parameter',
     N_('Some mandatory parameters are missing.')),
    (40002, 'api_bad_parameter',
     N_('Some parameters have invalid value specified.')),
]

g = globals()
error_codes = {}
for error_code, error_alias, error_message in error_codes_list:
    error_codes[error_code] = g['error_' + error_alias] = ErrorView(
        error_code, error_alias, error_message)
